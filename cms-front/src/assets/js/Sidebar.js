 ...
 */
Sidebar.prototype.gearImage = STENCIL_PATH + '';
...
Sidebar.prototype.addUmlPalette = function(expand)
{
	...
	this.addPaletteFunctions('uml', mxResources.get('uml'), expand || true, fns);
...
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview, cells, bounds)
{
	...
	dragSource.dragExit = function(graph, evt)
	{
		...
		//Angular Updates ....
		//If dropped Cell is a class 
		// Dispatch Custum Event to Angular with seleted cell as detail 
		var allCells = [];
		for (var c in  graph.getModel().cells){
			var myCell = graph.getModel().cells[c];
			allCells.push(myCell);
		}			
			var cell = graph.getSelectionCell();
			//Dispatch a custom event to get cell from Angular Service
			    var detail = {
					cell: cell,
					cellCount: allCells.length
				}
				window.dispatchCustomCellSelectionEvent(detail);
	
				/**
				 * Check if localStorage contains key: 'selected Classe'			 
				 * if true update selected cell by localstorage item
				 */
				window.addEventListener('storage', storageEventHandler, false);
				function storageEventHandler(evt){
					
				var updatedCell =  null;
				if(localStorage.hasOwnProperty("selectedClasse")){
				 updatedCell = localStorage.getItem("selectedClasse");
				   checkLocalStorageForCellUpdates(graph,cell, updatedCell);				 
				}		
	 ...
	 */
	 	...
	 	/**
				 ...
				 * if true update selected cell by localstorage item
				 */
				window.addEventListener('updateDiagramEvent', updateDiagramEventHandler, false);
				function updateDiagramEventHandler(evt){

				var updatedCell =  null;
				if(localStorage.hasOwnProperty("diagramXmlFile")){
				  localStorage.removeItem("diagramXmlFile");
				 //  checkLocalStorageForCellUpdates(graph,cell, updatedCell);				 
				}		
				localStorage.setItem('diagramXmlFile',mxUtils.getXml(ui.editor.getGraphXml()));

				//Dispatch a custom event to Tell Angular that diagram is updated and xml is ready in memory
				var diagramUpdatedEvent = new CustomEvent('onDiagramViewUpdated')
				window.dispatchEvent(diagramUpdatedEvent);
				}
	 /*****
	  * 
	  * 
...
// dispatch custom event suite à la séléction d'une cellule to get cell from Angular Service
function dispatchCustomCellSelectionEvent(detail) {
    var event = new CustomEvent('onJsCellCelect', { 'detail': detail })
	window.dispatchEvent(event);
}

function checkLocalStorageForCellUpdates(graph, cell, updatedCell){
	//Get Local Storage Item if Exists, Update Cell And refresh Graph
	  var model = graph.getModel();
	 
	  const storedObj = JSON.parse(updatedCell);
	  var storedAttributes = storedObj.attributes;
	  var storedOperations = storedObj.operations;

	  var designCellPrototype;
	  var cellPrototype;
		...
		{				
			...
			//Search For Cell To Update
			if(cell.getId() == storedObj.id){
				
				//Update Cell Value : Class Name
				graph.model.setValue(cell, storedObj.name);

				//Update Cell Children	
				var cellChildren = cell.children;	
				var lastChildAttribute = 0;
				var lastChildOperation = 0;
					//Get Last Attribute to push new Attribute After or delete deleted
					for (var l in cellChildren){
						var cc = cellChildren[l];

						//delete deleted Cells
						for(var d in storedObj.deletedCells){
							var cellTodelete = storedObj.deletedCells[d];
							if(cellTodelete.id == cc.id){
								graph.getModel().remove(cc);
							}
						}

                       if(cc.value.startsWith('-')){
						 this.lastChildAttribute = cell.getIndex(cc);
					   }
					   if(cc.value.startsWith('+')){
						this.lastChildOperation = cell.getIndex(cc);
					  }
					}		

				//Check and Update Attributes
				for (var s in storedAttributes){
					var storedAttribute = storedAttributes[s];

					var cellChildAttributeFound = cellChildren.find(c => c.getId() == storedAttribute.id);
					if(cellChildAttributeFound != undefined){
						
							graph.model.setValue(cellChildAttributeFound, '- '+ storedAttribute.name + ' : ' + storedAttribute.type);
							cellPrototype = graph.model.cloneCell(cellChildAttributeFound);
					  }
					  else{


						//var current = designCellPrototype.getGeometry();
						// current.width = 0;	
							cellPrototype.value = '- '+ storedAttribute.name + ' : ' +storedAttribute.type;	
							
							graph.addCell( cellPrototype, cell, ( this.lastChildAttribute + 1 ));
					  }
				}	
				
					//Check and Update Operations
					for (var s in storedOperations){
						var storedOperation = storedOperations[s];
						//Get a design cell prototype to add before every attribute found in local storage
						//var designCellPrototypeFound = cellChildren.find(c => c.value == "");						
	
						var cellChildOperationFound = cellChildren.find(c => c.getId() == storedOperation.id);

						if(cellChildOperationFound != undefined){
								graph.model.setValue(cellChildOperationFound, '+ '+ storedOperation.name +'('+ storedOperation.type +') : '+ storedOperation.returnType);
							//	cellPrototype = graph.model.cloneCell(cellChildOperationFound);
						  }
						else{
							//var current = designCellPrototype.getGeometry();
							// current.width = 0;	
								cellPrototype.value =  '+ '+ storedOperation.name +'('+ storedOperation.type +') : '+ storedOperation.returnType;	
								graph.addCell( cellPrototype, cell, ( this.lastChildOperation + 1 ) );
						  }
					}
			}
		}
		...
		//localStorage.removeItem("selectedClasse");
