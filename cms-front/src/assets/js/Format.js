...

{
	...
	console.log('regardez ici pour virer le prop panel');
	...
	if (graph.isSelectionEmpty())
	{
		...
		// Angular Updates Begin ...
		//If SelectionEmpty() remove selected cell from LocalStorage 
		if( graph.getSelectionModel().cells === undefined || graph.getSelectionModel().cells.length == 0){

			if (localStorage.getItem('selectedClasse') != null){
				localStorage.removeItem('selectedClasse');
			}		

			//Dispatch a custom event to Tell Angular that selection is empty
				var cEvent = new CustomEvent('onEmptySelection')
...
ArrangePanel.prototype.addGroupOps = function(div)
{
	...
	{
		let btn = mxUtils.button(mxResources.get('group'), function(evt)
		{
	...
	{
		const btn = mxUtils.button(mxResources.get('ungroup'), function(evt)
		{
   		...
   		graph.getModel().isVertex(graph.getModel().getParent(cell)))
	{
		...
		const btn = mxUtils.button(mxResources.get('removeFromGroup'), function(evt)
		{
	...
	else if (graph.getSelectionCount() > 0)
	{
		...
		const btn = mxUtils.button(mxResources.get('clearWaypoints'), mxUtils.bind(this, function(evt)
		{
